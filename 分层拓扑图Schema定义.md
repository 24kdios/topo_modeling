...
Layer 5: Service Layer (服务层)
  ↓ carried-by/carries
Layer 4: Tunnel Layer (隧道层)
  ↓ relies-on/supports
Layer 3: Routing Layer (路由层)
  ↓ uses-link/provides-path
Layer 2: Link Layer (链路层)
  ↓ connects/provides-link
Layer 1: Network Layer (网络层)
```

### 5.2 关系特征

1. **层内关系(Intra-Layer)**:
   - 包含关系(CONTAINS/BELONGS_TO): 描述层级结构
   - 连接关系(CONNECTS_TO/CONNECTED_BY): 描述物理和逻辑连接
   - 提供关系(PROVIDES/PROVIDED_BY): 描述能力提供
   - 成员关系(MEMBER_OF/HAS_MEMBER): 描述聚合和归属
   - 冗余关系(PRIMARY_BACKUP): 描述主备保护
   - 对等关系(PEER_WITH/STACKS_WITH): 描述对等互联

2. **层间关系(Inter-Layer)**:
   - 使用关系(USES/SERVES): 描述上层对下层资源的依赖
   - 承载关系(CARRIED_BY/CARRIES): 描述服务到隧道的映射
   - 依赖关系(RELIES_ON/SUPPORTS): 描述隧道到路由的依赖
   - 连接关系(CONNECTS/PROVIDES_LINK): 描述链路到设备的映射

3. **短接关系(Shortcut)**:
   - 层内短接: 优化层内查询,减少跳数
   - 层间短接: 优化层间依赖链查询
   - 跨层短接: 优化跨多层查询,实现空间换时间

### 5.3 设计原则

1. **IETF标准遵循**: 所有实体映射到RFC 8345的Network/Node/Link/TP拓扑类型
2. **分层抽象**: 6层独立建模,层间通过关系映射
3. **双向关系**: 关键关系定义正向和反向语义
4. **短接优化**: 高频查询路径建立直达关系
5. **冗余保护**: 关键资源定义主备关系
6. **运维属性**: 所有实体包含状态、时间、数据源属性
...
**索引创建**
```cypher
CREATE INDEX node_entity_id IF NOT EXISTS FOR (n:Node) ON (n.entity_id)
CREATE INDEX site_entity_id IF NOT EXISTS FOR (s:Site) ON (s.entity_id)
CREATE INDEX port_entity_id IF NOT EXISTS FOR (p:PhysicalPort) ON (p.entity_id)
CREATE INDEX link_entity_id IF NOT EXISTS FOR (l:Link) ON (l.entity_id)
CREATE INDEX tunnel_entity_id IF NOT EXISTS FOR (t:Tunnel) ON (t.entity_id)
CREATE INDEX vpn_entity_id IF NOT EXISTS FOR (v:VPNInstance) ON (v.entity_id)
CREATE INDEX business_entity_id IF NOT EXISTS FOR (b:BusinessService) ON (b.entity_id)
```

**约束创建**
```cypher
CREATE CONSTRAINT node_entity_id_unique IF NOT EXISTS FOR (n:Node) REQUIRE n.entity_id IS UNIQUE
CREATE CONSTRAINT site_entity_id_unique IF NOT EXISTS FOR (s:Site) REQUIRE s.entity_id IS UNIQUE
CREATE CONSTRAINT port_entity_id_unique IF NOT EXISTS FOR (p:PhysicalPort) REQUIRE p.entity_id IS UNIQUE
CREATE CONSTRAINT link_entity_id_unique IF NOT EXISTS FOR (l:Link) REQUIRE l.entity_id IS UNIQUE
CREATE CONSTRAINT tunnel_entity_id_unique IF NOT EXISTS FOR (t:Tunnel) REQUIRE t.entity_id IS UNIQUE
CREATE CONSTRAINT vpn_entity_id_unique IF NOT EXISTS FOR (v:VPNInstance) REQUIRE v.entity_id IS UNIQUE
CREATE CONSTRAINT business_entity_id_unique IF NOT EXISTS FOR (b:BusinessService) REQUIRE b.entity_id IS UNIQUE
```
...
**边类型定义**
```cpp
CREATE UNDIRECTED EDGE CONTAINS (
  FROM Node, 
  TO Rack,
  FROM Rack,
  TO Shelf,
...
CREATE DIRECTED EDGE CONNECTS_TO (
  FROM PhysicalPort,
  TO OpticalFiber,
  FROM TerminationPoint,
  TO Link
)
```
...
**业务服务到端口**
```cypher
MATCH (bs:BusinessService {entity_id: 'bs-001'})-[:USES_PORT]->(pp:PhysicalPort)
RETURN bs.business_service_name, pp.port_name
```

**VPN实例到隧道**
```cypher
MATCH (vpn:VPNInstance {entity_id: 'vpn-001'})-[:CARRIED_BY_TUNNEL]->(t:Tunnel)
RETURN vpn.vpn_instance_name, t.tunnel_name
```
...
**端口故障影响业务**
```cypher
MATCH (pp:PhysicalPort {entity_id: 'port-001'})-[:SERVES_BUSINESS]->(bs:BusinessService)
RETURN pp.port_name, bs.business_service_name
```

**节点故障影响VPN**
```cypher
MATCH (n:Node {entity_id: 'node-001'})-[:HOSTS_VPN]->(vpn:VPNInstance)
RETURN n.node_id, vpn.vpn_instance_name
```
...
**节点端口树**
```cypher
MATCH path = (n:Node {entity_id: 'node-001'})-[:CONTAINS*]->(pp:PhysicalPort)
RETURN path
```

**OSPF区域路由**
```cypher
MATCH (oa:OSPFArea {area_id: '0.0.0.0'})-[:GENERATES]->(r:Route)
RETURN oa.area_id, r.destination_prefix, r.next_hop
```

---

本图Schema定义完整描述了分层拓扑本体的实体和关系结构,可直接用于图数据库(Neo4j/TigerGraph)的Schema设计和实现。